...
test('Dokploy template renders its generated development contract when Docker Compose is available', {
  ...
  assert.equal(services.garage.user, '65532:65532')
  assert.equal(services.livekit.user, '65532:65532')
  assert.equal(services['livekit-egress'].user, 'egress')
  assert.equal(services.garage.configs[0].target, '/etc/garage.toml')
  assert.equal(services.livekit.configs[0].target, '/livekit.yaml')
  assert.equal(services['livekit-egress'].configs[0].target, '/livekit-egress.yaml')
  assert.match(rendered.configs.garage_config.content, /replication_factor = 1/)
  assert.match(rendered.configs.livekit_config.content, /udp_port: 7882/)
  assert.match(rendered.configs.livekit_egress_config.content, /endpoint: http:\/\/garage:3900/)
